...
def fetch_mlb_game_data(selected_date):
    """Fetches the Game ID, Abbreviations, Boxscore, and Play-by-Play data."""
...
if page == "Live Game":
    ...
    if plays:
        ...
        # --- AGGREGATE PITCH USAGE & COUNTS ---
        pitcher_pitches = []
        for p in plays:
            if p['about']['atBatIndex'] <= current_atBatIndex and p['matchup']['pitcher']['id'] == pitcher_id:
                pitch_events = [e for e in p.get('playEvents', []) if e.get('isPitch')]
